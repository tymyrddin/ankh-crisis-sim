...
class MapCanvas:
    def __init__(self, parent: tk.Frame, map_image_path: str | Path, width: int = 900, height: int = 700):
        ...
        self._image_refs: list = []  # keeps Tk images alive
        self._load_base_map(map_image_path)

        self._lamp_ids: dict[str, int] = {}
        self._glow_ids: dict[str, int] = {}

        self.on_building_hover: Callable[[str, int, int], None] | None = None
        self.on_building_leave: Callable[[], None] | None = None
        self.on_building_click: Callable[[str, int, int], None] | None = None
        ...
        for district in city.districts.values():
            for building in district.buildings.values():
                ...
                if building.hidden_failure:
                    colour = STATUS_COLOURS[BuildingStatus.OPERATIONAL]  # hidden failures still look green
    ...
    def _building_id_under_pointer(self) -> str | None:
    ...
    def _on_hover(self, event: tk.Event) -> None:
        bid = self._building_id_under_pointer()
